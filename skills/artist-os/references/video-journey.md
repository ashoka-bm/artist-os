...
- For cumulative or full long-form video, recommend Long-Work Stewardship when ADR 0013 applies and create the record only after the artist activates it; do not expand later dependent sequences, scenes, or shot batches while activation is unresolved, the initial Medium Mapping Checkpoint is unresolved for multiple dependent parts, or Long-Work Readiness is `repair_before_expansion` unless the artist repairs or explicitly waives the block.
...
If medium-level routing newly recommends `long_work_stewardship` and no activation decision exists, present the ADR 0015 activation gate before enrichment or bulk expansion. If the artist activates stewardship, enrich the active Long-Work Stewardship Record from the completed Video Medium Plan with one Long-Work Part per video sequence, scene, or other accepted dependent unit; include readiness, checkpoints, continuity rules, and drift management before expansion. If the artist defers or waives, record only the Gate Decision and do not create a placeholder stewardship record.
