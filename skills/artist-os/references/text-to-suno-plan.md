...
For cumulative or full long-form sound, the Long-Work Stewardship Record is the cumulative execution guard only after the artist activates it. Recommend it only when BOTH ADR 0013 conditions hold: (1) a cumulative arc across the tracks — Album Cohesion Mode `arc_album` for albums, otherwise dependent sequence parts where later tracks rely on earlier ones — AND (2) the audio length floor of full-length work (~8+ dependent tracks / ~30 minutes). An album or EP of individual songs should not recommend stewardship by default however large. If medium-level routing newly recommends `long_work_stewardship` and no activation decision exists, present the ADR 0015 activation gate before enrichment or bulk expansion. Activation creates the foundation record immediately; defer and waive are Gate Decisions only. Enrich active stewardship after Sound Medium Plan with track, movement, section, or sequence-part refs; do not duplicate arrangement or Sonic Dynamics fields inside the stewardship record.
