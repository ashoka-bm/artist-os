...
When image is activated as a **supporting medium** per the Cross-Medium Plan's Medium Role, plan a compact-tier Medium Plan that takes continuity from the **primary medium's realization** in addition to the Shared Story Spine; when image is the primary medium, plan at full depth.
...
For image series, the Long-Work Stewardship Record is the cumulative execution guard only after the artist activates it. If medium-level routing newly recommends `long_work_stewardship` and no activation decision exists, present the ADR 0015 activation gate before enrichment or bulk expansion. Activation creates the foundation record immediately; defer and waive are Gate Decisions only. Enrich active stewardship after Image Medium Plan, and center the enriched record on Long-Work Parts that reference `image_role_id` values. Do not duplicate Shot Design, amplitude profiles, or visual tension details inside the stewardship record; follow the Image Medium Plan refs when reviewing those details.
...
14. Record medium-level Workflow Scale Routing. A compact multi-beat arc can become a single `compressed_arc`. Per ADR 0013, add `long_work_stewardship` only when BOTH conditions hold: (1) recurring-subject continuity (dependent image roles where recurring characters, subjects, or world rules must persist across parts) AND (2) the image length floor of a book-scale continuous series (~20+ images), or an illustrated long-form work whose stewardship rides on the text floor. A portfolio or collection of individual images never activates it however large. Activate image-series and Long-Work supports only when `workflow_scale_routing` calls for them under this threshold; durable canon and full long-form image support still drive other supports.
...
20. When medium-level routing recommends `long_work_stewardship`, first resolve the ADR 0015 activation gate if no activation decision exists. If the artist activates stewardship, enrich the active Long-Work Stewardship Record from the completed Image Medium Plan with one Long-Work Part per image role or other accepted image-series/full-long-form part, continuity rules, checkpoint plan, and Long-Work Readiness before any multi-image prompt expansion. If the artist defers or waives, record only the Gate Decision and do not create a placeholder stewardship record.
