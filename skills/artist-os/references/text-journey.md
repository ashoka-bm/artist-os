...
For cumulative long text, the Long-Work Stewardship Record protects execution across text sections, chapters, scenes, or poem movements only after the artist activates it. If medium-level routing newly recommends `long_work_stewardship` and no activation decision exists, present the ADR 0015 activation gate before enrichment or bulk expansion. Activation creates the foundation record immediately; defer and waive are Gate Decisions only. Enrich active stewardship after Text Medium Plan by referencing text section or other medium part ids. Do not duplicate section execution, voice, fidelity, or editorial policy fields inside the stewardship record; those remain owned by Text Medium Plan and Text Generation Plan.
...
9. Record medium-level Workflow Scale Routing after the Medium Output Shape Recommendation is accepted or explicitly allowed to proceed unconfirmed and after any Medium Output Shape Conflict is resolved. A compact or structured single text artifact does not recommend Long-Work Stewardship just because it has several sections. Per ADR 0013, recommend `long_work_stewardship` only when BOTH conditions hold: (1) dependent text parts (later sections rely on earlier ones) AND (2) the text length floor of a multi-chapter work. A single-chapter piece, however many sections, never recommends it by default; recommend cumulative or full long-form supports only when dependent multi-chapter text, durable canon, extraction/verification, or long-form drafting across sessions are needed.
...
17. When medium-level routing recommends `long_work_stewardship`, first resolve the ADR 0015 activation gate if no activation decision exists. If the artist activates stewardship, enrich the active Long-Work Stewardship Record from the completed Text Medium Plan with one Long-Work Part per text section, chapter, scene, or poem movement; include readiness, checkpoints, continuity rules, and drift management before Draft Generation Approval. If the artist defers or waives, record only the Gate Decision and do not create a placeholder stewardship record. Plot-tracker-style Stewardship Views are projections over Text Medium Plan and Long-Work Stewardship state, not separate authority records.
