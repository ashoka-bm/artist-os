...
- If the packet omits required gate context for the artifact under review, fail closed: put the missing Symbology, Style, Prompt Lock, Text Form, Writing Method, Review Presentation, Draft Generation Approval, Vocal / Lyric, Arrangement / Form, Video Format, Shot Logic, Motion / Pacing / Transition, Audio Posture, Generation Approval, Output Acceptance, or other required gate in `emotional_tension_review.missing_context`, add a finding, and use `revise` or `block` when the missing gate prevents judging traceability, provider boundary, drafting readiness, or minimum tension.
