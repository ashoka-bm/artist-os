...
4. **Beat Plan** — medium skill; `schemas/beat-plan.schema.json`. When a reusable movement pattern would clarify the plan, read `docs/structure-library/README.md`, then `docs/structure-library/story/README.md`, then only the selected Story Structure entry; adapt it into `story_structure` on the Beat Plan rather than applying it unchanged. For writing/text and exploratory story development, preserve the strict `writing-beats` choice rhythm (2-3 candidate beats, artist chooses, one beat at a time); for image or Suno autopilot you may draft a full recommended Beat Plan. Each Beat names its intended feeling.
