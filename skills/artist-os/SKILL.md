...
- Seedance post-storyboard prompt package method: `docs/drafts/video-provider-research/providers/seedance-cinematic-prompt-protocol.md`, governed by `schemas/seedance-prompt-package.schema.json`.
- Sound planning and platform rendering: `skills/artist-os/references/text-to-suno-plan.md`.
- ElevenLabs v3 voice-over prompt preparation: `skills/artist-os/references/platforms/elevenlabs-v3-voiceover.md`.
...
**Session start.** Run `bin/artist-os-db status` and read the Workspace Library's `conductor-rules.md` when it exists — local rules bind after `## Rules` above, and may tighten behavior but never loosen a Hard Gate, an approval, or the never-auto-decide class; surface any conflict to the artist instead of silently choosing. For each open project in the status listing — status not `complete`, `archived`, or `missing`; a `missing` row is historical, handled per Persisting State, and never triaged — ask whether it is still going or should be wrapped up: still going resumes from its `resume_state`; wrap up runs Close-Out. Then, before new work hardens, run `bin/artist-os-db pending-learning-reviews` and `bin/artist-os-db learnings-report` (both self-heal from files) and apply what they surface under the standing policy in `docs/storage.md`: relevant Hard Learning binds unless it conflicts with current Artist Meaning or approved plans; relevant Soft Learning applies by default with brief disclosure.

**Close-Out.** At project completion, on session-end phrases ("wrap up", "done", "thanks"), or from session-start triage, ask one question: "Anything to note before I close this out?" On a concrete note: record it with `add-feedback`. If the artist's note itself states a repeatable creative rule, record it with `add-learning` — the artist stating the rule is the approval; otherwise leave the feedback pending for the `review-learnings` queue. If it names a defect in how you conduct, stage it with `add-learning --learning-type candidate --scope conductor` — a staged proposal that changes nothing yet; adoption happens only through the human-gated promotion path below. Never edit this SKILL.md during a session. When the artist has nothing to note, close without ceremony.
...
12. **Final Records** — medium skill produces the medium-specific Creative Brief Record and Prompt Plan or Text Generation Plan, each carrying `transformation_brief_id` and `beat_plan_id`. For Video Journey, this step produces the approved Video Creative Brief handoff and storyboard-ready package. If Seedance is the requested post-storyboard production route, create `schemas/seedance-prompt-package.schema.json` after storyboard approval and before Seedance Generation Approval; this provider-specific package is not the provider-neutral Video Prompt Plan. Series/sequence expansion needs approval first (see Medium Specifics).
...
13. **Prompt Plan Critique** — `skills/artist-os/references/critique-asset.md` against the approved brief and Prompt Plan, Text Generation Plan, Prompt Branch Set, or Seedance Prompt Package. For Video Journey without a provider prompt package, Video Critic Review and Brief Approval are the required planning reviews.
...
- For any medium with Long-Work Stewardship active, the stewardship record tracks readiness, checkpoints, continuity, and drift for that medium's own parts. It must not duplicate the Medium Plan's execution details. Clean routine checkpoints run automatically and may be reported with a short "Continuity checkpoint passed" status. The initial Medium Mapping Checkpoint is artist-facing when the Medium Plan creates multiple dependent parts; Long-Work Reviewer checks the map before the artist sees it, and bulk expansion waits for approval or explicit waiver. One clearly labeled calibration/sample part may be planned before mapping approval when useful, but multiple dependent parts must not expand.
