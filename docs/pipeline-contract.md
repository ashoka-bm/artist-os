...
- Gate: Reference Strategy Gate (the `character_reference_strategy` / `visual_reference_sheet_strategy` gate decisions) for promotion and partial acceptance; Generation Approval Gate before image generation; Output Acceptance Gate before a reference output becomes `accepted`. Reference Readiness is a precondition check, not a recorded Gate Decision: required reference outputs must be `accepted` or `waived` before storyboard, image-series, or illustration prompt export.
