...
We mirror G-Stack's cheap, proven patterns: one data config per host, an all-hosts list with the host set derived from it, co-located validators exercised by a test, a single `usesEnvVars` switch for the anchor, a primary host with empty rewrites, and a config-to-bash bridge so the installer reads the registry instead of hardcoding paths. We deliberately skip its heavy machinery — no `.tmpl`/placeholder engine, no frontmatter transform DSL (allowlist/denylist/conditionalFields/renameFields/toolRewrites), no host-adapter escape hatch — which a prior audit ruled overkill for 11 skills across 2 hosts. The registry is JSON rather than TypeScript because the runtime is Python + bash with no TS toolchain; a `validate` subcommand plus a Python test recover the config-validation guarantee.

## Structure

- **Anchor.** Skill bodies add one sentence: *"Paths like `THEORY.md` and files under `docs/` and `schemas/` resolve from `$ARTIST_OS_ROOT` — the repo root in a checkout, the bundle root in a Codex install. If a referenced file is missing, run `bin/artist-os-paths doctor`."* The existing bare path strings are left unchanged, so the contract tests that assert on skill-body substrings keep passing.
- **Resolver.** `bin/artist-os-paths` (Python, modeled on `bin/artist-os-db`) with subcommands `root | get <host> <key> | doctor | list-hosts | validate`, emitting shell-escaped output so the bash installer can consume it.
...
- The anchor sentence in every reference-bearing skill body (9 skills: the conductor, `text-journey`, `text-to-image-plan`, `text-to-suno-plan`, the three reviewers, plus `ingest-reference` and `meaning-interview`) — bare path strings untouched.
- `packaging/hosts.json`, `packaging/MANIFEST.json`, `packaging/README.md`.
- Installer materializes the bundle for distributed installs (symlink-from-checkout stays the developer default), reads Codex's prefix/target via `artist-os-paths get codex`, and runs `doctor` post-install and in CI to fail loud on orphaned references — while the literal `skills=( ... )` array and the `install_skill "$repo_root/skills/artist-os"` line stay byte-intact so `test_skill_set_sync` and the install guardrails are preserved.
...
- **At-risk tests are handled.** `test_phase_order_doc_drift` untouched; `test_skill_set_sync` preserved (no skill churn; array, install line, references, and eval set unchanged); the five `*_contract` tests keep their valid path constants with the anchor sentence kept off the asserted substrings; `test_version_changelog_consistency` handled with a coordinated VERSION bump to `0.2.0` plus a CHANGELOG entry. The registry/manifest directory is named `packaging/` rather than `dist/` to signal hand-authored source, not build output.
