...
- Implement the sibling Artist Library / Workspace Library storage model: `WONDERMINT_ROOT`, Project Pointer files, visible-missing detection, Project Manifest fields for visible paths and feedback review status, SQLite indexes for visible paths / learning / performance signals, and sync behavior for human-edited Artist Library files.
