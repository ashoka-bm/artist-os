...
- Every **Text Medium Plan** must include a **Medium Output Shape Recommendation**, even when the recommended shape is obvious or compact; image plans may include the same recommendation object as optional rationale, while sound, video, and mixed-media adoption remain follow-up decisions.
...
- **Medium Output Shape Recommendation** is required in the **Text Medium Plan** and optional in the **Image Medium Plan**. In image work, it explains the choice among `single_image`, `compressed_arc`, and `image_series`; `presentation_mode` remains the accepted concrete image shape consumed downstream.
